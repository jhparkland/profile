...
function circumcircle(t, e, i, s) {
    var o,
        ...
        I = Math.abs(_ - k),
        b = Math.abs(k - P);
    if (I < EPSILON && b < EPSILON) 
        throw Error("Eek! Coincident points!");
    return I < EPSILON
        ? (
            h = -((y - x) / (P - k)),
            f = (x + y) / 2,
...
for (o = Array(g), i = g; i--;) 
    ...
    for (
        ...
        a = supertriangle(t),
        t.push(a[0], a[1], a[2]),
        l = [circumcircle(t, g + 0, g + 1, g + 2)],
        h = [],
        $ = [],
        i = o.length; i--; $.length = 0
    ) {
        for (p = o[i], s = l.length; s--;) {
            if ((f = t[p][0] - l[s].x) > 0 && f * f > l[s].r) {
                h.push(l[s]),
                l.splice(s, 1);
                continue
            }
            f * f + (d = t[p][1] - l[s].y) * d - l[s].r > EPSILON || (
                $.push(l[s].i, l[s].j, l[s].j, l[s].k, l[s].k, l[s].i),
                l.splice(s, 1)
            )
        }
        for (dedup($), s = $.length; s;) 
            v = $[--s],
            u = $[--s],
            l.push(circumcircle(t, u, v, p))
    }
    for (i = l.length; i--;) 
        h.push(l[i]);
    for (l.length = 0, i = h.length; i--;) 
        h[i].i < g && h[i].j < g && h[i].k < g && l.push(h[i].i, h[i].j, h[i].k);
    return l
}
function init() {
    for (
    ...
    for (t = 0; t < vertices.length; t++) 
        3 == o.length && (triangles.push(o), o = []),
        o.push(vertices[t]);
    for (t = 0; t < particles.length; t++) 
        for (e = 0; e < triangles.length; e++) 
            -1 !== (i = triangles[e].indexOf(t)) && triangles[e].forEach(
                function (e, i, s) {
                    e !== t && -1 == particles[t]
                        .neighbors
                        .indexOf(e) && particles[t]
                        .neighbors
                        .push(e)
                }
            );
if (renderFlares) 
        for (t = 0; t < flareCount; t++) 
            flares.push(new Flare);
"ontouchstart" in document.documentElement && window.DeviceOrientationEvent
...
function render() {
    if (
        randomMotion && (++n >= noiseLength && (n = 0), nPos = noisePoint(n)),
        context.clearRect(0, 0, canvas.width, canvas.height),
        blurSize > 0 && (context.shadowBlur = blurSize, context.shadowColor = randomColor(colorPalette)),
        renderParticles
    ) 
...
if (renderMesh) {
    ...
    if (renderLinks) {
        ...
        for (var h = links.length - 1; h >= 0; h--) 
            links[h] && !links[h].finished
                ? links[h].render()
                : delete links[h]
        }
    if (renderFlares) 
        for (var $ = 0; $ < flareCount; $++) 
            flares[$].render()
}
function resize() {
...
function startLink(t, e) {
    links.push(new Link(t, e))
}
...
Particle.prototype.render = function () {
    var t = position(this.x, this.y, this.z),
        e = (this.z * particleSizeMultiplier + particleSizeBase) * (sizeRatio() / 1e3),
        i = this.opacity;
    if (flicker) {
        var s = random(-.5, .5, !0);
...
Flare.prototype.render = function () {
    var t = position(this.x, this.y, this.z),
        e = (this.z * flareSizeMultiplier + flareSizeBase) * (sizeRatio() / 1e3);
    context.beginPath(),
...
function position(t, e, i) {
    return {
        x: t * canvas.width + (
            canvas.width / 2 - mouse.x + (nPos.x - .5) * noiseStrength
        ) * i * motion,
        y: e * canvas.height + (
            canvas.height / 2 - mouse.y + (nPos.y - .5) * noiseStrength
        ) * i * motion
    }
}
function sizeRatio() {
    ...
    switch (this.stage) {
        case 0:
            ...
            if (this.verts.length >= this.length) {
                for (t = 0; t < this.verts.length - 1; t++) {
                    var l = particles[this.verts[t]],
                        h = particles[this.verts[t + 1]],
                        $ = l.x - h.x,
                        f = l.y - h.y,
                        d = Math.sqrt($ * $ + f * f);
                    this
        ...
        case 1:
            if (this.distances.length > 0) {
                for (t = 0, s = []; t < this.linked.length; t++) 
                    i = position((e = particles[this.linked[t]]).x, e.y, e.z),
                    s.push([i.x, i.y]);
                ...
                if (this.traveled >= v) 
                    ...
                    this
                        .linked
                        .push(this.verts[this.linked.length]),
                    i = position((e = particles[this.linked[this.linked.length - 1]]).x, e.y, e.z),
                    s.push([i.x, i.y]),
                    this.linked.length >= this.verts.length && (this.stage = 2);
                else {
                    var p,
                        g = particles[this.linked[this.linked.length - 1]],
                        _ = particles[this.verts[this.linked.length]],
                        x = v - this.traveled,
                        k = (this.traveled * _.x + x * g.x) / v;
                    i = position(
                        k,
                        (this.traveled * _.y + x * g.y) / v,
                        (this.traveled * _.z + x * g.z) / v
                    ),
                    s.push([i.x, i.y])
                }
                this.drawLine(s)
        ...
        case 2:
            if (this.verts.length > 1) {
                if (this.fade < linkFade) {
                    ...
                    for (t = 0; t < this.verts.length; t++) 
                        i = position((e = particles[this.verts[t]]).x, e.y, e.z),
                        s.push([i.x, i.y]);
                    this.drawLine(s, y)
                } else 
                    this.stage = 3,
...
Link.prototype.drawLine = function (t, e) {
    if ("number" != typeof e && (e = linkOpacity), t.length > 1 && e > 0) {
        ...
        for (var i = 0; i < t.length - 1; i++) 
            context.moveTo(t[i][0], t[i][1]),
            context.lineTo(t[i + 1][0], t[i + 1][1]);
