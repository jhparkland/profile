...
document.addEventListener('DOMContentLoaded', function () {
    ...
    Array
        .prototype
        .forEach
        .call(firstNavs, function (nav_first) {
            if (page_tree[1] === nav_first.ariaLabel) {
                ...
                Array
                    .prototype
                    .forEach
                    .call(secondNavs, function (nav_second) {
                        if (page_tree[2] === nav_second.ariaLabel) {
                            ...
                            Array
                                .prototype
                                .forEach
                                .call(thirdNavs, function (nav_third) {
    ...
    document.addEventListener('click', function (e) {
        ...
        while (
            target && !(target.classList && target.classList.contains('nav-list-expander'))
        ) {
    ...
    themeButton.forEach((btn) => {
        btn.addEventListener('click', function () {
            ...
            const codeblocks = innerContent != null
                ? innerContent.querySelectorAll('pre')
                : null;
    ...
    if (cancelButton) {
        cancelButton.addEventListener('click', function () {
            ...
            Array
                .from(document.querySelectorAll('.result-item'))
...
function searchPost(pages) {
    document
        ...
        .addEventListener('keyup', function () {
            ...
            for (var i = 0; i < pages.length; i++) {
                ...
                if (post.title === 'Home' && post.type == 'category') 
                    continue;
                
                if (post.title.toLowerCase().indexOf(keyword) >= 0 || post.path.toLowerCase().indexOf(keyword) >= 0 || post.tags.toLowerCase().indexOf(keyword) >= 0) {
            ...
            for (var i = 0; i < matchedPosts.length; i++) {
                ...
                if (matchedPosts[i].type === 'post') {
                    ...
                    insertItem(
                        '<a href="' + matchedPosts[i].url + '"><table><thead><tr><th><svg class="ico-bo' +
                        'ok"></svg></th><th>' + highlighted_title + '</th></tr></thead><tbody><tr><td><' +
                        'svg class="ico-folder"></svg></td><td>' + highlighted_path + '</td></tr><tr><t' +
                        'd><svg class="ico-tags"></svg></td><td>' + highlighted_tags + '</td></tr><tr><' +
                        'td><svg class="ico-calendar"></svg></td><td>' + matchedPosts[i].date + '</td><' +
                        '/tr></tbody></table></a>'
                    );
                } else {
                    insertItem(
                        '<a href="' + matchedPosts[i].url + '"><table><thead><tr><th><svg class="ico-fo' +
                        'lder"></svg></th><th>' + highlighted_path + '</th></tr></thead></table></a>'
                    );
                }
            }
    ...
    function highlightKeyword(txt, keyword) {
        ...
        if (index >= 0) {
            out = txt.substring(0, index) + "<span class='highlight'>" + txt.substring(
                index,
                index + keyword.length
            ) + "</span>" + txt.substring(index + keyword.length);
            return out;
        }
...
function searchRelated(pages) {
    ...
    var currPost = pages.find(obj => {
        return obj.url === location.pathname
    });

    let currTags = currPost
        .tags
        .split(', ');
    let currCategory = currPost
        .path
        .split(' > ')
        .pop();
    ...
    for (var i = 0; i < Math.min(relatedPosts.length, 6); i++) {
        ...
        contents
            .classList
            .add("related-item");
        contents.innerHTML = '<a href="' + post.url + '"><img src="' + post.thumbnail +
                '"/><p class="category">' + category + '</p><p class="title">' + post.title + '</p><p class="date">' +
                date + '</p></a>';
